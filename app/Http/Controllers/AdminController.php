...
class AdminController extends Controller
{
    ...
    public function saveSettings(Request $request)
    {
        if ($request->hasFile('favicon')) {
            $request->validate([
                'favicon' => 'image|mimes:ico,png,jpg,jpeg|max:4096'
            ]);
        ...
        if ($request->hasFile('logo')) {
            $request->validate([
                'logo' => 'image|mimes:png,jpg,jpeg,svg|max:4096'
            ]);
            if (!file_exists(public_path('uploads/site'))) {
                mkdir(public_path('uploads/site'), 0755, true);
            }
            $request->file('logo')->move(public_path('uploads/site'), 'logo.png');
        }

        foreach ($request->except(['_token', 'favicon', 'logo']) as $key => $value) {
