...
class ComparisonController extends Controller
{
    ...
    public function compare(Request $request)
    {
        ...
        set_time_limit(120); // Increase timeout for comparison
        ...
        try {
            ...
            // Tambah data GDP dengan error handling
            try {
                $econA = $this->worldBank->getEconomicIndicators($codeA);
            } catch (\Exception $e) {
                \Log::warning("World Bank API failed for {$codeA}: " . $e->getMessage());
            ...
            try {
                ...
                \Log::warning("World Bank API failed for {$codeB}: " . $e->getMessage());
                $econB = ['gdp' => null, 'gdp_growth' => null, 'inflation' => null];
            }
            ...
            // Currency rates dengan error handling
            ...
            try {
                ...
                \Log::warning("Currency API failed for {$codeA}: " . $e->getMessage());
                $comparison['country_a']['currency_rate'] = null;
            }
            ...
            try {
                ...
                \Log::warning("Currency API failed for {$codeB}: " . $e->getMessage());
                $comparison['country_b']['currency_rate'] = null;
            }
            ...
            \Log::error("Comparison failed for {$codeA} vs {$codeB}: " . $e->getMessage());
            
            if ($request->ajax()) {
                return response()->json([
                    'error' => true,
                    'message' => 'Terjadi kesalahan saat membandingkan negara. Silakan coba lagi.'
                ], 500);
            }
            
            return back()->with('error', 'Terjadi kesalahan saat membandingkan negara. Silakan coba lagi.');
        }
    }
}
